...
| `--no-brief` | Disable the agent-facing sandbox brief written to the scratch dir (`CPLT_BRIEF.md`, on by default) and, with it, the opt-in `sandbox.agents_md` AGENTS.md block. Also `sandbox.brief = false` in config |
